...
「やったこと」はTogglの開始時刻が早い順、つまりその日に作業した順で表示します。

勤務場所として登録されたカレンダー予定と、Notion Calendarなどが作る `Busy`
（ブロック予定）は既定で日報から除外します。設定ページではこの自動除外を切り替え、
さらに日報へ出さないタイトルを1行ずつ完全一致で登録できます。既定の除外タイトルは
`朝タスク` です。タイトル除外はGoogleカレンダーだけでなく、Toggl・Notion・JSONの
全取得元に適用され、対象データはClaudeへ送りません。
